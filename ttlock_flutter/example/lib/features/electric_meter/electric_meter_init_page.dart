...
class ElectricMeterInitPage extends HookConsumerWidget {
  const ElectricMeterInitPage({
    super.key,
    required this.mac,
    required this.name,
  });
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    useEffect(() {
      ...
      final endpoint =
          config.defaultServerEndpoint(DeviceServerKind.electricMeter);
    ...
    Future<void> initDevice() async {
      ...
      final notifier = ref.read(electricMeterNotifierProvider.notifier);
      await notifier.connect(mac);
      if (!context.mounted) return;
      if (ref.read(electricMeterNotifierProvider).error != null) return;
      ...
      final resultText = ref.read(electricMeterNotifierProvider).result ?? '';
      ...
      if (!context.mounted) return;
      ElectricMeterRoute(meterId).go(context);
    }

    final isConfigValid = config?.isValid ?? false;

    return Scaffold(
      appBar: AppBar(title: const Text('Initialize Electric Meter')),
          ...
          : Form(
              ...
              child: ListView(
                ...
                children: [
                  ...
                  SectionHeader(title: 'Device', icon: Icons.bolt),
                  ...
                  if (config != null) ...[
                    ...
                    ServerEndpointFields(
                      config: config,
                      deviceKind: DeviceServerKind.electricMeter,
                  ...
                  ElevatedButton(
                    ...
                    child: const Text('Initialize Electric Meter'),
                  ),
                ],
              ),
            ),
