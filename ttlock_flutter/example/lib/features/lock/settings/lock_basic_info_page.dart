...
class LockBasicInfoPage extends ConsumerStatefulWidget {
  const LockBasicInfoPage({super.key, required this.lockMac});

  final String lockMac;

  @override
  ConsumerState<LockBasicInfoPage> createState() => _LockBasicInfoPageState();
}

class _LockBasicInfoPageState extends ConsumerState<LockBasicInfoPage> {
  ...
  Widget build(BuildContext context) {
    ...
    final statusAsync = ref.watch(lockStatusProvider(widget.lockMac));
    final capsAsync = ref.watch(lockCapabilitiesProvider(widget.lockMac));
    ...
    return Scaffold(
      ...
      body: lockAsync.when(
        ...
        data: (lock) {
          if (lock == null) {
            return const ErrorDisplay(message: 'Lock not found');
          }
          if (!_initialized) {
            _nameController.text = lock.name;
            _initialized = true;
          }
          return ListView(
            ...
            children: [
                ...
                onPressed: () async {
                  await runSettingsOperation(
                    ...
                    action: () => ref.read(lockListNotifierProvider.notifier).updateDevice(
                          lock.copyWith(name: _nameController.text.trim()),
                        ),
              ...
              FilledButton.tonal(
                onPressed: () async {
                  ...
                  await runSettingsOperation(
                    context,
                    action: () => ref.read(lockApiProvider).setLockTime(
                          DateTime.now().millisecondsSinceEpoch ~/ 1000,
                          lockData,
                        ),
                    successMessage: 'Lock time synced',
                  );
                  ref.invalidate(lockStatusProvider(widget.lockMac));
                },
                  ...
                  return FilledButton(
                    onPressed: () async {
                      final code = await runSettingsOperation(
                        context,
                        action: () => ref
                            .read(lockApiProvider)
                            .getAdminPasscode(lock.lockData),
                      );
              ...
              const SizedBox(height: 16),
              FutureBuilder(
                future: ref.read(lockApiProvider).getLockSystemInfo(lock.lockData),
