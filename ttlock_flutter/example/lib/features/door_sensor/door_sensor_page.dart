              ...
              children: [
                Card(
                  child: ListTile(
                    ...
                    title: Text('Door Sensor', style: AppTextStyles.titleMedium),
                ...
                SectionHeader(title: 'Standalone Mode', icon: Icons.play_arrow),
                const SizedBox(height: 8),
                if (widget.lockData != null)
                  _ActionButton(
                    icon: Icons.link,
                    label: 'Init Door Sensor (with lock)',
                    onTap: () => ref.read(doorSensorNotifierProvider.notifier).initDoorSensor(widget.mac, widget.lockData!),
                  ),
                _ActionButton(
                  icon: Icons.power_settings_new,
                  label: 'Init Standalone',
                  onTap: () => ref.read(doorSensorNotifierProvider.notifier).initStandalone(widget.mac, {'type': 0}),
                ),
                _ActionButton(
                  icon: Icons.info_outline,
                  label: 'Read Feature Value',
                  onTap: () => ref.read(doorSensorNotifierProvider.notifier).readFeatureValue(widget.mac),
                ),
                const SizedBox(height: 16),
                Row(
                  children: [
                    SizedBox(
                      ...
                      child: TextField(
                        ...
                        decoration: const InputDecoration(labelText: 'Function #', isDense: true),
                    ...
                    Expanded(
                      child: _ActionButton(
                        ...
                        label: 'Check Support',
                        onTap: () {
                          ref.read(doorSensorNotifierProvider.notifier).checkSupport(
                            widget.mac,
                            '',
