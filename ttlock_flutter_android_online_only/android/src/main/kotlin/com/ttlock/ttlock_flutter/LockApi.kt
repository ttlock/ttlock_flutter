...
class LockApi: TTLockHostApi {
    ...
    private fun mapLongValue(data: Map<String, Any?>, key: String): Long {
        ...
        return when (value) {
            ...
            else -> throw IllegalArgumentException("Missing long value for key: $key")
        }
    }
    ...
    override fun getAllValidPasscodes(lockData: String, callback: (Result<List<TTPasscodeModel>>) -> Unit) {
        TTLockClient.getDefault().getAllValidPasscodes(lockData, object : GetAllValidPasscodeCallback {
            override fun onGetAllValidPasscodeSuccess(passcodeStr: String) {
                callback.invoke(
                    Result.success(
                        parseJsonListMaps(passcodeStr).map {
                            TTPasscodeModel(
                                keyboardPwd = it["keyboardPwd"] as String,
                                newKeyboardPwd = it["newKeyboardPwd"] as String,
...
//        })
    ...
    override fun getAllValidCards(lockData: String, callback: (Result<List<TTICCardModel>>) -> Unit) {
        TTLockClient.getDefault().getAllValidICCards(lockData, object : GetAllValidICCardCallback {
            override fun onGetAllValidICCardSuccess(cardListStr: String) {
                callback.invoke(Result.success(parseJsonListMaps(cardListStr).map {
                    TTICCardModel(
    ...
    override fun getAllValidFingerprints(lockData: String, callback: (Result<List<TTFingerprintModel>>) -> Unit) {
        TTLockClient.getDefault().getAllValidFingerprints(lockData, object : GetAllValidFingerprintCallback {
            override fun onGetAllFingerprintsSuccess(fingerprintStr: String) {
                callback.invoke(Result.success(parseJsonListMaps(fingerprintStr).map {
                    TTFingerprintModel(
                        fingerprintNumber = it["fingerprintNumber"] as String,
                        startDate = mapLongValue(it, "startDate"),
                        endDate = mapLongValue(it, "endDate"),
                    )
                }))
